...
module sha512_top (
    ...
    word_t w_i, k_i; // current word and current round constant
    ...
    sha512_msg_sched u_sched (
        ...
        .w_out      (w_i)             // w[0] is sent to round module for operation
    );
    ...
    always_ff @(posedge clk or negedge rst_n) begin
        if (!rst_n) begin
            ...
            case (state)
                ...
                WORK: begin 
                    round_count <= round_count + 1; // every round, increment round count
                    {a,b,c,d,e,f,g,h} <= {a_n,b_n,c_n,d_n,e_n,f_n,g_n,h_n}; // load hash regs with next values
                    if (round_count == 79) state <= ACCUM; // after 80 rounds (on one block) move to accumulate stage
                end
                ACCUM: begin // after 80 rounds (on one block)
                    ...
                    H[6] <= H[6] + g; 
                    H[7] <= H[7] + h;
                    // output of the current block = initial values (in H) + output of the block (a-h)
    ...
    assign k_i = K[round_count]; // current round's constant from the pkg
